...
export class CredentialStore {
  ...
  save(account: string, password: string) {
    try {
      if (!this.useFallback) this._saveNative(account, password);
  ...
  get(account: string): string | null {
    try {
      if (!this.useFallback) return this._getNative(account);
  ...
  delete(account: string) {
    try {
      if (!this.useFallback) this._deleteNative(account);
  ...
  private _saveNative(account: string, password: string) {
    if (this._isMac()) this._saveMac(account, password);
    else if (this._isWindows()) this._saveWindows(account, password);
    else if (this._isLinux()) this._saveLinux(account, password);
    else throw new Error('Unsupported platform');
  }

  private _getNative(account: string): string | null {
    if (this._isMac()) return this._getMac(account);
    else if (this._isWindows()) return this._getWindows(account);
    else if (this._isLinux()) return this._getLinux(account);
    else throw new Error('Unsupported platform');
  }

  private _deleteNative(account: string) {
    if (this._isMac()) this._deleteMac(account);
    else if (this._isWindows()) this._deleteWindows(account);
    else if (this._isLinux()) this._deleteLinux(account);
    else throw new Error('Unsupported platform');
  }
